...
{
    public class WsService : NetServiceBase<NetTcpBinding>, IServerService
    {
        string _hostFormat = "http://{0}:{1}/";

        string _serverFormat = "http://{0}:{1}/{2}";

        public WsService(string ip, string port) : base(ip, port)
        {
        ...
        {
            this.Register<I, T, WSHttpBinding>(new Uri(string.Format(_hostFormat, _ip, _port)), l =>
              {
